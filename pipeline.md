...
[7] 环境反射辐亮度：每面片 (8) 式计算 E_i → L_refl（可选，默认关闭）
[8] 总辐亮度：L_total = L_self + L_refl
[9] 能量衰减：光学系统衰减 (4) → L_out
[10] 灰度转换：辐亮度 → DN（线性映射）
[11] 统计 + 渲染
```
...
&\quad \times \bigg\{
x^3 + \frac{3}{\dfrac{C_2}{T} \cdot \left[ x^2 + \dfrac{2}{\dfrac{C_2}{T} \cdot \left( x + \dfrac{1}{C_2/T} \right)} \right]}
\bigg\} \Bigg|_{x=1/\lambda_1}^{x=1/\lambda_2}
\quad\text{(7)}
\end{aligned}
}
$$
...
**此模块默认关闭**（`ENV_RADIATION_ENABLED = False`）。开启时 $L_{\text{total}} = L_{\text{self}} + L_{\text{refl}}$；关闭时 $L_{\text{total}} = L_{\text{self}}$。
...
前序步骤得到的是物理辐亮度 $L^{\text{detected}}$（W/(m²·sr)），最终红外图像需要将其映射为像素灰度值（DN, Digital Number）。我们采用简单的线性映射——将辐亮度范围线性拉伸到目标灰度范围。

### 14.1 线性映射

对每个面片的辐亮度 $L_i$，映射到目标灰度范围 $[DN_{\min}, DN_{\max}]$：

$$
DN_i = DN_{\min} + \frac{L_i - L_{\min}}{L_{\max} - L_{\min}} \cdot (DN_{\max} - DN_{\min})
$$

其中 $L_{\min}$、$L_{\max}$ 为当前帧所有面片辐亮度的最小值和最大值（动态自适应），$DN_{\min}$、$DN_{\max}$ 为输出灰度范围（如 0–255 或 0–65535）。

### 14.2 与管线的衔接

1. 管线输出辐亮度 $L^{\text{detected}}$（已叠加 §13.1 能量衰减和 §13.2 探测器噪声）
2. 统计 $L_{\min}$、$L_{\max}$，按 14.1 线性映射到灰度值
3. Clamp 到 $[DN_{\min}, DN_{\max}]$，得到最终红外图像
