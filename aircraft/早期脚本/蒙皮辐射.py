...
def apply_thermal_material(obj):
    """应用热辐射材质到对象，基于辐射强度而非温度"""
    # 创建新材质
    mat_name = "RadiationIntensityMaterial"
    ...
    if len(color_ramp.color_ramp.elements) < 3:
        # 添加新的控制点
        for _ in range(3 - len(color_ramp.color_ramp.elements)):
...
def main():
    ...
    min_y = min(v.co.y for v in vertices) if vertices else 0
    max_y = max(v.co.y for v in vertices) if vertices else 0
    y_range = max(max_y - min_y, 0.1)  # 防止除以零
    
    print(f"物体Y轴范围: {min_y:.2f} 到 {max_y:.2f} (范围: {y_range:.2f})")
    
    # 存储所有辐射强度值用于范围调整
    radiation_values = []
    
    # 设置温度分布：随着Y值增加而升高（正Y方向为尾焰方向）
    # 同时计算并存储辐射强度
    for i, vert in enumerate(vertices):
        # 计算Y轴位置比例 (0到1)
        y_factor = (vert.co.y - min_y) / y_range
        
        # 限制在0-1范围内
        y_factor = max(0.0, min(1.0, y_factor))
        
        # 计算温度 (从300K到800K)
        temperature = 300 + y_factor * 500
        ...
        # 计算辐射强度
        rad_value = calculate_IM(temperature)
    ...
    print(f"辐射强度范围: {min_rad:.3e} 到 {max_rad:.3e} W/sr")
    
    # 应用辐射强度材质
    mat = apply_thermal_material(obj)
    ...
    if vertices:
        ...
        bottom_IM = mesh.attributes["Radiation"].data[0].value
        
        # 顶部顶点
        top_temp = mesh.attributes["Temperature"].data[-1].value
        top_IM = mesh.attributes["Radiation"].data[-1].value
        
        # 中间顶点
        mid_idx = len(vertices) // 2
        mid_temp = mesh.attributes["Temperature"].data[mid_idx].value
        mid_IM = mesh.attributes["Radiation"].data[mid_idx].value
    ...
    print("红外热成像设置完成！现在材质基于实际辐射强度。")
    print("提示：通过修改min_rad/max_rad设置辐射强度的显示范围")
